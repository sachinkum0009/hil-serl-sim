...
def main():
    ...
    parser.add_argument(
        "--step-size", type=float, default=0.01, help="Step size for movement in meters"
    )
    parser.add_argument(
        "--render-mode",
        type=str,
    ...
    parser.add_argument(
        "--use-keyboard", action="store_true", help="Use keyboard control"
    )
    parser.add_argument(
        "--reset-delay",
        type=float,
    ...
    env_id = (
        ...
        else "gym_hil/PandaPickCubeGamepad-v0"
    )
    env = gym.make(
        env_id,
        render_mode=args.render_mode,
